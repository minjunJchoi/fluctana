...
############################# spectral methods #############################
    ...
    def fftbins(self, nfft, window, overlap, tau=0, detrend=0, full=0):
        ...
        for d, D in enumerate(self.Dlist):
            ...
            for c in range(cnum):
                x = D.data[c,:]
                D.ax, D.spdata[c,:,:], D.win_factor = sp.fftbins(x, dt, nfft, window, overlap, tau=tau, detrend=detrend, full=full)
    ...
    def fftmulti(self, window, overlap=0, detrend=0, full=0):
        ...
        for d, D in enumerate(self.Dlist):
            ...
            for c in range(cnum):
                for t in range(bins):
                    x = D.multi_data[c,t,:]
                    D.ax, D.spdata[c,t,:], D.win_factor = sp.fftbins(x, dt, nfft, window, overlap, tau=0, detrend=detrend, full=full)
...
############################# default plot functions ###########################
    ...
    def iplot(self, dnum=0, snum=0, plot_type='time', istep=0.005, 
        ...
        # 2D plot boundary setting
        bdry = [1e4, -1e4, 1e4, -1e4] 
        for _, D in enumerate(self.Dlist):
        ...
        def plot2D(axs, tidx, D, clear=True):
            ...
            # fill bad channel data with neighbor average
